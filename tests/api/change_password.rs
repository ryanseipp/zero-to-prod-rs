...
#[tokio::test]
async fn new_password_must_be_greater_than_12_characters() {
    let app = spawn_app().await;
    let new_password = Uuid::new_v4().to_string();
    ...
    let response = app
        .post_change_password(&serde_json::json!({
            "current_password": &app.test_user.password,
            "new_password": &new_password[..11],
            "new_password_check": &new_password[..11]
        }))
        .await;
    assert_is_redirect_to(&response, "/admin/password");
...
async fn new_password_must_be_less_than_128_characters() {
    let app = spawn_app().await;
    let new_password: String = Password(128..129).fake();
